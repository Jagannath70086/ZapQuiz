...
export default async function QuizViewer() {
  ...
  const getDifficultyColor = (difficulty) => {
    switch (difficulty?.toLowerCase()) {
      case 'easy': return 'bg-green-500/20 text-green-800 dark:text-green-300 border-green-300/50';
      case 'medium': return 'bg-yellow-500/20 text-yellow-800 dark:text-yellow-300 border-yellow-300/50';
      case 'hard': return 'bg-red-500/20 text-red-800 dark:text-red-300 border-red-300/50';
      default: return 'bg-gray-500/20 text-gray-800 dark:text-gray-300 border-gray-300/50';
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-12 relative z-10">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-amber-800/80 dark:text-amber-200/80 max-w-2xl mx-auto">
            Challenge yourself with our curated collection of quizzes across various topics and difficulty levels
          </p>
        </div>
                ...
                >
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      {q.difficulty && (
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full border ${getDifficultyColor(q.difficulty)}`}>
                          {q.difficulty}
                        </span>
                      )}
        ...
        <div className="text-center py-8">
          <div className="backdrop-blur-md bg-white/20 dark:bg-orange-900/30 rounded-2xl p-6 border border-amber-200/30 dark:border-amber-700/30 inline-block">
            <p className="text-amber-800/80 dark:text-amber-200/80">
              Found <span className="font-bold text-amber-700 dark:text-yellow-400">{quiz.length}</span> quizzes across <span className="font-bold text-amber-700 dark:text-yellow-400">{Object.keys(categorized).length}</span> categories
            </p>
          </div>
        </div>
